...
$joinQuery = "FROM `tbl_job_list` AS `u`
    ...
    LEFT JOIN `tbl_feedback_type` AS `ud` ON (`ud`.`idtbl_feedback_type` = `u`.`tbl_itenary_type_tblid_itenary_type`)
    LEFT JOIN `tbl_feedback` AS `ue` ON (`ue`.`idtbl_feedback` = `u`.`tbl_feedback_tblid_feedback`)";
